...
export class FffEditor extends CustomEditor {
	// The running app's TUI/theme instances come from the pi-tui copy bundled
	// inside pi-coding-agent, not this workspace's hoisted (older) pi-tui.
	// Deriving ctor types from typeof CustomEditor keeps both sides on one identity.
	private readonly runtime: FffRuntime;
