...
import { completeOutsidePath, isAnchoredPathQuery } from "./outside-search.ts";
import {
...
// expandHome collapses a trailing separator ("~/" resolves to the home dir
// itself), losing the "list this directory" shape; re-attach it when the raw
// query carried one so completeOutsidePath lists contents instead of
// matching the leaf name in the parent.
function preserveTrailingSeparator(rawQuery: string, normalizedQuery: string): string {
	let value = rawQuery.trim();
	if (value.startsWith("@")) value = value.slice(1);
	if (value.startsWith('"') && value.endsWith('"') && value.length >= 2) value = value.slice(1, -1);
	value = value.trimEnd();
	return !normalizedQuery.endsWith("/") && (value.endsWith("/") || value === "..") ? `${normalizedQuery}/` : normalizedQuery;
}
...
export class FffRuntime {
	...
	async trackQuery(query: string, selectedPath: string): Promise<AppResult<void, RuntimeInitializationError | FinderOperationError>> {
		const finderResult = await this.ensure();
		if (finderResult.isErr()) return propagateError(finderResult);
		// Candidates are relative to the index root, not necessarily process.cwd().
		const selectedAbsolutePath = resolve(this.basePath, normalizePathQuery(selectedPath));
		return toVoidResult(safeFinderCall("trackQuery", () => finderResult.value.trackQuery(normalizePathQuery(query), selectedAbsolutePath)));
	}
	...
	async searchFileCandidates(query: string, limit = DEFAULT_FILE_CANDIDATE_LIMIT): Promise<AppResult<FffFileCandidate[], RuntimeInitializationError | FinderOperationError>> {
		...
		// Anchored (`~`, `/`, `../`, `./`) queries point outside the project
		// index; serve them from the filesystem directly instead of spinning
		// up or querying the native finder for paths it can never hold.
		if (isAnchoredPathQuery(normalizedQuery)) {
			return Result.ok(await completeOutsidePath(this.cwd, preserveTrailingSeparator(query, normalizedQuery), limit));
		}
		const finderResult = await this.ensure();
		if (finderResult.isErr()) return propagateError(finderResult);
