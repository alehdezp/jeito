...
describe("createStore", () => {
  ...
  test("read-only mode does not initialize or persist query caches", async () => {
    const dbPath = freshDbPath();
    const writer = await createStore({ dbPath, config: { collections: {} } });
    await writer.close();
    const before = await stat(dbPath, { bigint: true });

    const reader = await createStore({ dbPath, readOnly: true });
    expect(await reader.listCollections()).toEqual([]);
    expect(() => reader.internal.setCachedResult("query", "result")).toThrow(/read-only|readonly/i);
    await reader.close();
    const after = await stat(dbPath, { bigint: true });
