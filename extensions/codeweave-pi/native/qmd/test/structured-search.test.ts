...
describe("structuredSearch", () => {
  ...
  test("uses candidateLimit for backend retrieval breadth before fusion", async () => {
    const search = vi.spyOn(store, "searchFTS").mockReturnValue([]);
    await structuredSearch(store, [{ type: "lex", query: "test" }], { candidateLimit: 37 });
    expect(search).toHaveBeenCalledWith("test", 37, undefined);
    search.mockRestore();
  });
