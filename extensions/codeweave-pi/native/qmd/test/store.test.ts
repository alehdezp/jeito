...
describe("Embedding batching", () => {
  ...
  test("generateEmbeddings stops all remaining document batches after catastrophic provider failure", async () => {
    const store = await createTestStore();
    const db = store.db;
    const fakeLlm = createFakeEmbedLlm();
    let embedCalls = 0;
    fakeLlm.embed = (async () => {
      embedCalls++;
      if (embedCalls === 1) return { embedding: [0.1, 0.2, 0.3], model: "fake-embed" };
      throw new Error("provider unavailable");
    }) as typeof fakeLlm.embed;
    fakeLlm.embedBatch = (async (texts: string[]) => {
      fakeLlm.embedBatchCalls.push([...texts]);
      throw new Error("provider unavailable");
    }) as typeof fakeLlm.embedBatch;
    ...
    try {
      for (let index = 0; index < 80; index++) {
        await insertTestDocument(db, "docs", { name: `failure-${index}`, body: `# Failure ${index}\n\nMarker ${index}` });
      }

      const result = await generateEmbeddings(store, { maxDocsPerBatch: 64, maxBatchBytes: 1024 * 1024 });

      expect(fakeLlm.embedBatchCalls).toHaveLength(1);
      expect(embedCalls).toBeLessThan(80);
      expect(result.errors).toBe(32);
