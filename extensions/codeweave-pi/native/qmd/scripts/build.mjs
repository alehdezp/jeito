...
import { chmodSync, cpSync, readFileSync, renameSync, rmSync, writeFileSync } from "node:fs";
...
const runtimeTmpPath = `${runtimePath}.tmp`;
rmSync(runtimeTmpPath, { recursive: true, force: true });
cpSync(join(root, "dist"), runtimeTmpPath, { recursive: true });
rmSync(runtimePath, { recursive: true, force: true });
renameSync(runtimeTmpPath, runtimePath);
