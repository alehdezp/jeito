...
function initializeSqliteVec(db) {
    try {
        loadSqliteVec(db);
    ...
    const result = await withLLMSessionForLlm(llm, async (session) => {
        ...
        for (const batchMeta of batches) {
            ...
            for (const doc of batchDocs) {
                ...
                const chunks = await chunkDocumentByTokens(doc.body, undefined, undefined, undefined, doc.path, options?.chunkStrategy, session.signal, llm);
...
export function createStore(dbPath, options = {}) {
    ...
    const store = {
        ...
        expandQuery: (query, model, intent) => expandQuery(query, model ?? store.llm?.generateModelName ?? DEFAULT_QUERY_MODEL, db, intent, store.llm, !options.readOnly),
        rerank: (query, documents, model, intent) => rerank(query, documents, model ?? store.llm?.rerankModelName ?? DEFAULT_RERANK_MODEL, db, intent, store.llm, !options.readOnly),
        // Document retrieval
 ...
 */
export async function chunkDocumentByTokens(content, maxTokens = CHUNK_SIZE_TOKENS, overlapTokens = CHUNK_OVERLAP_TOKENS, windowTokens = CHUNK_WINDOW_TOKENS, filepath, chunkStrategy = "regex", signal, llmOverride) {
...
// Query expansion
// =============================================================================
export async function expandQuery(query, model = DEFAULT_QUERY_MODEL, db, intent, llmOverride, writeCache = true) {
...
// Reranking
// =============================================================================
export async function rerank(query, documents, model = DEFAULT_RERANK_MODEL, db, intent, llmOverride, writeCache = true) {
        ...
        if (vecSearches.length > 0) {
            ...
            for (let i = 0; i < vecSearches.length; i++) {
                ...
                if (!embedding)
                    continue;
                for (const coll of collectionList) {
                    const vecResults = await store.searchVec(vecSearches[i].query, embedModel, candidateLimit, coll, undefined, embedding);
                    if (vecResults.length > 0) {
                        for (const r of vecResults)
                            docidMap.set(r.filepath, r.docid);
                        rankedLists.push(vecResults.map(r => ({
