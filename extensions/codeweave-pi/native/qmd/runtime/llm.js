 ...
 */
export function isZeroEntropyEmbeddingModel(modelUri) {
...
export function formatQueryForEmbedding(query, modelUri) {
    ...
    if (isZeroEntropyEmbeddingModel(uri))
        return query;
...
export class LlamaCpp {
     ...
     */
    async ensureLlama(allowBuild = this.allowModelDownloads) {
        if (!this.llama) {
    ...
    async resolveModel(modelUri) {
        if (!this.allowModelDownloads) {
            ...
            if (!modelPath)
                throw new Error(`QMD model is not cached for read-only inference: ${modelUri}`);
            return modelPath;
        }
