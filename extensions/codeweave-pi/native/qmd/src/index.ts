...
export { ZeroEntropyProvider, type ZeroEntropyProviderOptions } from "./zeroentropy.js";
export { VoyageProvider, type VoyageProviderOptions } from "./voyage.js";
...
export interface StoreOptions {
  ...
  /** Optional inference provider; defaults to the local node-llama-cpp implementation. */
  llm?: LLM;
  /** Open the existing index read-only and disable persistent query caches. */
  readOnly?: boolean;
}
...
export async function createStore(options: StoreOptions): Promise<QMDStore> {
  ...
  const internal = createStoreInternal(options.dbPath, { readOnly: options.readOnly });
  ...
  // Use an injected provider when supplied; otherwise preserve QMD's local default.
