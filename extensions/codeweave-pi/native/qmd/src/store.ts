...
/**
 * Get the configured inference provider for a store, falling back to the local provider.
 */
...
function initializeSqliteVec(db: Database): void {
  try {
    loadSqliteVec(db);
...
export type Store = {
  db: Database;
  dbPath: string;
  /** Optional inference provider for this store (overrides the local singleton). */
  llm?: LLM;
  close: () => void;
...
export async function generateEmbeddings(
  ...
  const result = await withLLMSessionForLlm(llm, async (session) => {
    ...
    let abortRemainingBatches = false;
    ...
    for (const batchMeta of batches) {
      ...
      for (let batchStart = 0; batchStart < batchChunks.length; batchStart += BATCH_SIZE) {
        ...
        if (processed >= BATCH_SIZE && activeErrorCount() > processed * 0.8) {
          abortRemainingBatches = true;
          console.warn(`⚠ Error rate too high (${activeErrorCount()}/${processed}) — stopping remaining embedding batches`);
          break;
        }
      ...
      if (!abortRemainingBatches) await retryFailedChunks(true);
      ...
      bytesProcessed += batchBytes;
      options?.onProgress?.({ chunksEmbedded, totalChunks, bytesProcessed, totalBytes, errors: activeErrorCount(), failures: failureList() });
      if (abortRemainingBatches) break;
    }
 ...
 */
export function createStore(dbPath?: string, options: { readOnly?: boolean } = {}): Store {
  const resolvedPath = dbPath || getDefaultDbPath();
  const db = openDatabase(resolvedPath, options.readOnly ? { readonly: true } : undefined);
  if (options.readOnly) {
    initializeSqliteVec(db);
  ...
  const store: Store = {
    ...
    // Query expansion & reranking
    expandQuery: (query: string, model?: string, intent?: string) => expandQuery(query, model ?? store.llm?.generateModelName ?? DEFAULT_QUERY_MODEL, db, intent, store.llm, !options.readOnly),
    rerank: (query: string, documents: { file: string; text: string }[], model?: string, intent?: string) => rerank(query, documents, model ?? store.llm?.rerankModelName ?? DEFAULT_RERANK_MODEL, db, intent, store.llm, !options.readOnly),
...
export async function chunkDocumentByTokens(
  ...
  if (!llm.tokenize || !llm.detokenize) {
    return charChunks.map(chunk => ({ ...chunk, tokens: Math.ceil(chunk.text.length / avgCharsPerToken) }));
  }
  const tokenize = llm.tokenize.bind(llm);
  const detokenize = llm.detokenize.bind(llm);
...
export async function expandQuery(query: string, model: string = DEFAULT_QUERY_MODEL, db: Database, intent?: string, llmOverride?: LLM, writeCache: boolean = true): Promise<ExpandedQuery[]> {
  ...
  if (writeCache && expanded.length > 0) {
...
export async function rerank(query: string, documents: { file: string; text: string }[], model: string = DEFAULT_RERANK_MODEL, db: Database, intent?: string, llmOverride?: LLM, writeCache: boolean = true): Promise<{ file: string; score: number }[]> {
  ...
  for (const search of searches) {
    if (search.type === 'lex') {
      for (const coll of collectionList) {
        const ftsResults = store.searchFTS(search.query, candidateLimit, coll);
        if (ftsResults.length > 0) {
          for (const r of ftsResults) docidMap.set(r.filepath, r.docid);
          rankedLists.push(ftsResults.map(r => ({
            file: r.filepath, displayPath: r.displayPath,
  ...
  if (hasVectors) {
    ...
    if (vecSearches.length > 0) {
      ...
      for (let i = 0; i < vecSearches.length; i++) {
        ...
        for (const coll of collectionList) {
          const vecResults = await store.searchVec(
            vecSearches[i]!.query, embedModel, candidateLimit, coll,
            undefined, embedding
          );
          if (vecResults.length > 0) {
            for (const r of vecResults) docidMap.set(r.filepath, r.docid);
