...
type NodeLlamaCppModule = {
  ...
  resolveModelFile: (model: string, optionsOrDirectory: string | { directory: string; cli?: boolean }) => Promise<string>;
  LlamaChatSession: new (options: { contextSequence: unknown }) => {
    prompt: (prompt: string, options?: Record<string, unknown>) => Promise<string>;
  };
  LlamaLogLevel: { disabled?: unknown; error: unknown };
};
 ...
 */
export function isZeroEntropyEmbeddingModel(modelUri: string): boolean {
  return /^zeroentropy\/zembed-1(?:\/|$)/i.test(modelUri);
}
...
export function formatQueryForEmbedding(query: string, modelUri?: string): string {
  ...
  if (isZeroEntropyEmbeddingModel(uri)) return query;
...
export function formatDocForEmbedding(text: string, title?: string, modelUri?: string): string {
  const uri = modelUri ?? resolveEmbedModel();
  if (isZeroEntropyEmbeddingModel(uri)) return title ? `${title}\n${text}` : text;
  if (isQwen3EmbeddingModel(uri)) {
...
export async function pullModels(
  ...
  for (const model of models) {
    ...
    const path = await resolveModelFile(model, { directory: cacheDir, cli: options.cli ?? Boolean(process.stdout.isTTY) });
...
export type LlamaCppConfig = {
  ...
  /**
   * Allow native backend/model downloads and source builds (default: true).
   * Read-only query paths set this false so missing prepared artifacts fail
...
export class LlamaCpp implements LLM {
  ...
  private allowModelDownloads: boolean;
  ...
  constructor(config: LlamaCppConfig = {}) {
    ...
    this.allowModelDownloads = config.allowModelDownloads ?? true;
   ...
   */
  private async ensureLlama(allowBuild = this.allowModelDownloads): Promise<Llama> {
    if (!this.llama) {
      ...
      const loadLlama = async (gpu: LlamaGpuMode, sourceBuildAllowed = allowBuild, buildOverride?: "auto" | "never") =>
        await withNativeStdoutRedirectedToStderr(() => getLlama({
          ...
          // Native initialization can emit error-colored Metal compiler noise
          // even when the packaged runtime succeeds. JS catch paths below own
          // actionable failure diagnostics, so successful initialization stays quiet.
          logLevel: LlamaLogLevel.disabled ?? LlamaLogLevel.error,
          gpu,
          progressLogs: false,
  ...
  private async resolveModel(modelUri: string): Promise<string> {
    if (!this.allowModelDownloads) {
      const filename = modelUri.startsWith("hf:") ? modelUri.split("/").pop() : undefined;
      const candidates = filename && existsSync(this.modelCacheDir)
        ? readdirSync(this.modelCacheDir, { withFileTypes: true })
            .filter(entry => entry.isFile() && entry.name.includes(filename))
            .map(entry => join(this.modelCacheDir, entry.name))
        : [modelUri];
      const modelPath = candidates.find(candidate => inspectGgufFile(candidate).valid);
      if (!modelPath) throw new Error(`QMD model is not cached for read-only inference: ${modelUri}`);
      return modelPath;
    }
