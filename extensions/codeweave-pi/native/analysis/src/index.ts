...
import { refreshGoInterfaceEdges } from './resolution/callback-synthesizer';
...
export class CodeGraph {
  ...
  async sync(options: IndexOptions = {}): Promise<SyncResult> {
    return this.indexMutex.withLock(async () => {
      try {
        ...
        // A skipped sync cannot become codeweave-pi's completed-run receipt.
        throw new Error('Maintenance sync could not acquire the donor file lock');
      }
      ...
      try {
        ...
        // Derived Go satisfaction depends on method sets, not just edge-endpoint
        // files. Scoped resolution alone leaves unchanged type→interface edges
        // stale after method edits/removals. Reuse the existing Go passes only.
        if (filesChanged || result.filesRemoved > 0) {
          await refreshGoInterfaceEdges(this.queries);
        }
  ...
  /** Internal admitted-maintenance adapter only; do not open another SQL owner. */
