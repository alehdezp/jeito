...
// FileLock accesses the bound private store; source helpers stay admission-gated.
import * as storageFs from 'node:fs';
...
export class FileLock {
  ...
  acquire(): void {
    ...
    if (storageFs.existsSync(this.lockPath)) {
      try {
        const content = storageFs.readFileSync(this.lockPath, 'utf-8').trim();
        const pid = parseInt(content, 10);
        const stat = storageFs.statSync(this.lockPath);
