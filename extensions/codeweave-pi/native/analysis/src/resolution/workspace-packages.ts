...
function readOhpmMain(projectRoot: string, dirRel: string): string | null {
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:98');
    return null;
  }
...
function collectOhpmFileDeps(projectRoot: string): Map<string, string> {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:146');
      continue;
    }
...
function readOhpmFileDeps(manifestAbs: string): Array<[string, string]> {
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:187');
    return out;
  }
...
function readWorkspaceGlobs(projectRoot: string): string[] {
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:248');
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:257');
...
function expandWorkspaceGlob(projectRoot: string, pattern: string): string[] {
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:305');
    return [];
  }
  const out: string[] = [];
  for (const e of entries) {
...
function readPackageName(dirAbs: string): string | null {
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/workspace-packages.ts:321');
    return null;
  }
}
