...
import { matchReference, matchFunctionRef, matchDottedCallChain, matchScopedCallChain, matchMethodCall, sameLanguageFamily, sameMethodLanguage, crossesKnownFamily, dumpNameMatcherProfile, clearNameMatcherMemos } from './name-matcher';
...
const PASCAL_UNIT_PREFIXES = [
  ...
  'Soap.', 'Xml.', 'websift.', 'REST.', 'FireDAC.', 'IBX.',
  'IdHTTP', 'IdTCP', 'IdSSL',
];
...
export class ReferenceResolver {
  ...
  runPostExtract(): number {
    ...
    for (const fw of this.frameworks) {
      ...
      try {
        ...
        throw new Error(`Required post-extract pass failed: ${fw.name}`, { cause: err });
      }
    }
  ...
  private readFileCached(filePath: string): string | null {
    ...
    try {
      ...
      assertCaughtFailure(error, 'src/resolution/index.ts:423');
  ...
  private createContext(): ResolutionContext {
    return {
      ...
      getExportEdges: (filePath: string) => this.context.getNodesInFile(filePath)
        .filter((node) => node.kind === 'file')
        .flatMap((node) => this.queries.getOutgoingEdges(node.id, ['exports'])),
      ...
      getMethodMatches: (typeName: string, methodName: string, language: Language) => {
        ...
        if (typeName.includes('::') || methodName.includes(':')) {
          ...
          for (const m of candidates) {
            ...
            if (!sameMethodLanguage(m.language, language)) continue;
      ...
      fileExists: (filePath: string) => {
        if (!fs.existsSync(path.join(this.projectRoot, filePath))) return false;
        ...
        try {
          ...
          assertCaughtFailure(error, 'src/resolution/index.ts:545');
      ...
      getFileLines: (filePath: string) => {
        if (!fs.existsSync(path.join(this.projectRoot, filePath))) return null;
      ...
      getAllFiles: () => {
        return this.queries.getAllFilePaths().filter(file => fs.existsSync(path.join(this.projectRoot, file)));
      },
      ...
      listDirectories: (relativePath: string) => {
        ...
        try {
          ...
          assertCaughtFailure(error, 'src/resolution/index.ts:577');
  ...
  resolveOne(ref: UnresolvedRef): ResolvedRef | null {
    ...
    // A TS/JS constructor qualified by an import or receiver cannot become an
    // unrelated global name after its binding fails. Keep the existing import
    // resolver, but make its miss final for these construction references.
    if (ref.referenceKind === 'instantiates' &&
        ['typescript', 'tsx', 'javascript', 'jsx'].includes(ref.language) &&
        (ref.referenceName.includes('.') || this.matchesAnyImport(ref))) {
  ...
  async resolveAndPersistBatched(
    ...
    try {
      ...
      throw new Error('Required connection synthesis failed', { cause: error });
    }
