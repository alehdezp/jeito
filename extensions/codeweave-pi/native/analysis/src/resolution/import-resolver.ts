...
import * as fs from '../../capture';
import { assertCaughtFailure } from '../../capture';
 ...
 * per referencing symbol otherwise). Legacy named/component lookup keeps its
 * array order; TS/JS defaults require a unique persisted export target instead.
 */
interface FileExportIndex {
  byName: Map<string, Node>;
  defaultComponent: Node | undefined;
  defaultDeclaration: Node | undefined;
}
...
function getFileExportIndex(filePath: string, context: ResolutionContext): FileExportIndex {
  ...
  if (!idx) {
    idx = { byName: new Map(), defaultComponent: undefined, defaultDeclaration: undefined };
    ...
    for (const n of nodes) {
      ...
      if (idx.defaultComponent === undefined && n.kind === 'component' &&
          (n.language === 'svelte' || n.language === 'vue')) idx.defaultComponent = n;
    }
    const fileIds = new Set(nodes.filter((n) => n.kind === 'file').map((n) => n.id));
    const defaults = new Set((context.getExportEdges?.(filePath) ?? [])
      .filter((edge) => edge.kind === 'exports' && fileIds.has(edge.source) &&
        edge.provenance === 'tree-sitter' && edge.metadata?.exportedName === 'default')
      .map((edge) => edge.target));
    if (defaults.size === 1) {
      idx.defaultDeclaration = nodes.find((n) => defaults.has(n.id) && n.kind !== 'file');
    }
...
function loadCppIncludeDirsFromCompileDB(projectRoot: string): string[] | null {
  ...
  for (const c of candidates) {
    try {
      ...
      assertCaughtFailure(error, 'src/resolution/import-resolver.ts:557');
      // ignore
    }
  }
  ...
  try {
    ...
    assertCaughtFailure(error, 'src/resolution/import-resolver.ts:604');
    return null;
  }
}
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        assertCaughtFailure(error, 'src/resolution/import-resolver.ts:670');
        // ignore permission errors
      }
    }
  } catch (error) {
    assertCaughtFailure(error, 'src/resolution/import-resolver.ts:674');
    // ignore
  }
      ...
      if (resolvedPath) {
        ...
        if (targetNode) {
          ...
          if (!imp.isNamespace && ref.referenceName.startsWith(imp.localName + '.')) {
            ...
            // A constructor member that is absent is not the imported base
            // class, nor an instance method inferred from loose source text.
            if (ref.referenceKind === 'instantiates' &&
                ['typescript', 'tsx', 'javascript', 'jsx'].includes(ref.language)) return null;
...
function findExportedSymbolWalk(
  ...
  if (want.isDefault) {
    // TS/JS defaults require a persisted parser-backed binding. Merely being
    // the first exported class/function/component is not default-export proof.
    // Svelte/Vue retain their existing implicit component default semantics.
    const direct = exportIndex.defaultDeclaration ?? exportIndex.defaultComponent;
