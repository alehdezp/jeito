...
/** JS/TS module methods cross parser dialects; other language gates stay exact. */
export function sameMethodLanguage(candidate: string, caller: string): boolean {
  return candidate === caller || (LANGUAGE_FAMILY[candidate] === 'web' && LANGUAGE_FAMILY[caller] === 'web');
}
/**
...
function isLexicallyReachable(
  ...
  if (ref.language === 'go' && ref.referenceKind === 'calls'
      && (candidate.kind === 'method' || candidate.kind === 'property' || candidate.kind === 'field')
      && /^[\p{L}_][\p{L}\p{Nd}_]*$/u.test(ref.referenceName)
      && Number.isSafeInteger(ref.line) && ref.line > 0
      && Number.isSafeInteger(ref.column) && ref.column >= 0) {
    ...
    if (line !== undefined && ref.column <= line.length) {
      // Carrier columns are UTF-16, just like JS string offsets. Extractors
      // sometimes erase a receiver; only the captured spelling proves bare syntax.
      const carrier = line.slice(ref.column);
      if (carrier.startsWith(ref.referenceName)
          && /^\s*\(/.test(carrier.slice(ref.referenceName.length))) return false;
    }
  }
...
export function resolveMethodOnType(
  ...
  if (context.getMethodMatches) {
    ...
    for (const m of methodCandidates) {
      ...
      if (!sameMethodLanguage(m.language, ref.language)) continue;
...
export function matchMethodCall(
  ...
  // TS/JS field chains now retain their receiver. If the existing typed
  // resolution above cannot establish it, do not guess an enclosing class's
  // same-named method. Import/qualified-name resolution still runs first.
  if (LANGUAGE_FAMILY[ref.language] === 'web' && dotMatch && objectOrClass!.includes('.')) {
    return null;
  }
...
export function matchFuzzy(
  ...
  const callableCandidates = applyLanguageGate(candidates.filter((n) => callableKinds.has(n.kind)
    && (ref.language !== 'go' || isLexicallyReachable(n, ref, context))), ref);
