...
import { goMethodSignatureKey } from './go-method-signature';
 ...
 * matching method names and syntactically consistent signatures. The dispatch
 * bridge ({@link interfaceOverrideEdges}, now 'go'-enabled) can link an interface
 * method call to the concrete overrides.
 *
 * Alias/import binding and complete method-set semantics remain heuristic;
 * contradictory or unavailable signatures cannot establish satisfaction. Empty
 * interfaces (`any`) are skipped so they don't match every struct.
 */
async function goImplementsEdges(queries: QueryBuilder, onYield: MaybeYield): Promise<Edge[]> {
  ...
  const methodSignatures = (id: string): Map<string, string | undefined> => {
    const methods = new Map<string, string | undefined>();
    for (const edge of queries.getOutgoingEdges(id, ['contains'])) {
      const method = queries.getNodeById(edge.target);
      if (!method || method.kind !== 'method') continue;
      const signature = method.signature ?? '';
      if (!signatures.has(signature)) signatures.set(signature, goMethodSignatureKey(signature));
      const key = signatures.get(signature);
      methods.set(method.name, methods.has(method.name) && methods.get(method.name) !== key ? undefined : key);
    }
    return methods;
  };
  ...
  for (const iface of queries.iterateNodesByKind('interface')) {
    if ((++scanned255 & 63) === 0) await onYield();
    if (iface.language === 'go') goInterfaces.push(iface);
  }
  if (goStructs.length === 0 || goInterfaces.length === 0) return edges;
  // Bulk extraction may have loaded Go only in workers. Reuse the local grammar
  // loader; missing grammar/signatures withhold this optional inference.
  await loadGrammarsForLanguages(['go']);
  const structMethods = new Map<string, Map<string, string | undefined>>();
  for (const s of goStructs) structMethods.set(s.id, methodSignatures(s.id));

  for (const iface of goInterfaces) {
    if ((++scanned255 & 63) === 0) await onYield();
    const want = methodSignatures(iface.id);
    ...
    for (const s of goStructs) {
      ...
      let all = true;
      for (const [name, signature] of want) {
        if (!signature || have.get(name) !== signature) { all = false; break; }
      }
      if (!all) continue;
...
async function interfaceOverrideEdges(queries: QueryBuilder, onYield: MaybeYield, onlyGo = false): Promise<Edge[]> {
  let scanned255 = 0;
  const edges: Edge[] = [];
  const seen = new Set<string>();
  ...
  for (const cls of queries.iterateNodesByKind(kind)) {
    if ((++scanned255 & 63) === 0) await onYield();
    if (onlyGo && cls.language !== 'go') continue;
...
/** Sync's scoped reference resolver does not run whole-graph synthesis. Refresh
 * only this derived Go closure: a method edit can invalidate an implements edge
 * whose two endpoints live in an unchanged types file. No source is reindexed.
 */
export async function refreshGoInterfaceEdges(queries: QueryBuilder): Promise<void> {
  if (!queries.getDistinctFileLanguages().has('go')) return;
  const yieldToLoop = createYielder();
  queries.deleteGoInterfaceEvidence();
  queries.insertEdges(await goCrossFileMethodContainsEdges(queries, yieldToLoop));
  queries.insertEdges(await goImplementsEdges(queries, yieldToLoop));
  queries.insertEdges(await interfaceOverrideEdges(queries, yieldToLoop, true));
}
