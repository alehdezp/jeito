...
// Database/schema/sidecar IO only; project source reads belong to capture.ts.
import * as fs from 'fs';
import * as path from 'path';
import { SchemaVersion } from '../types';
import { runMigrations, getCurrentVersion, CURRENT_SCHEMA_VERSION } from './migrations';
import { maintenanceStore } from '../../storage';
import { MAINTENANCE_OUTPUT_ARTIFACTS } from '../../identity.mjs';
...
export class DatabaseConnection {
  ...
  static initialize(dbPath: string): DatabaseConnection {
    ...
    const schemaPath = path.join(__dirname, MAINTENANCE_OUTPUT_ARTIFACTS.schema);
...
export function getDatabasePath(projectRoot: string): string {
  return maintenanceStore(projectRoot).database;
}
