...
export class QueryBuilder {
    ...
    if (!this.stmts.updateNode) {
      this.stmts.updateNode = this.db.prepare(`
        UPDATE nodes SET
          ...
          semantic_input_digest = NULL
        WHERE id = @id
      `);
    }

    // Invalidate cache before update
    // Projection depends on the owner's kind and extents. INSERT OR REPLACE
    // already clears its binding; in-place updates must invalidate it too.
    this.nodeCache.delete(node.id);
  ...
  *iterateNodeNames(): IterableIterator<string> {
  ...
  /** Retire only the Go interface pass's derived facts before recomputing them. */
  deleteGoInterfaceEvidence(): void {
    this.db.prepare(`DELETE FROM edges
      WHERE provenance = 'heuristic' AND source IN (SELECT id FROM nodes WHERE language = 'go')
        AND CASE WHEN json_valid(metadata) THEN json_extract(metadata, '$.synthesizedBy') END
          IN ('go-implements', 'interface-impl')`).run();
  }
