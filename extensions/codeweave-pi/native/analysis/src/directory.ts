...
import { maintenanceStore } from '../storage';
...
export function getCodeGraphDir(projectRoot: string): string {
  return maintenanceStore(projectRoot).directory;
}
...
export function isInitialized(projectRoot: string): boolean {
  return fs.existsSync(maintenanceStore(projectRoot).database);
}
...
export function createDirectory(projectRoot: string): void {
  if (fs.existsSync(maintenanceStore(projectRoot).database)) {
    throw new Error(`CodeGraph already initialized in ${projectRoot}`);
  }
  // The shared private-store owner created/validated the directory. No source
  // .codegraph directory or repository .gitignore is created here.
}
...
export function removeDirectory(projectRoot: string): void {
  maintenanceStore(projectRoot);
  throw new Error('Private maintenance store removal is not exposed');
}
