...
import * as fsp from '../../capture';
import { existsSync as packageExistsSync } from 'node:fs';
...
export function scanDirectory(
  ...
  // Custom extension → language overrides from the project's codegraph.json.
  const overrides = loadExtensionOverrides(rootDir);
  const admitted = fs.admittedSourceFiles(rootDir);
  if (admitted) {
    const files = admitted.filter(file => isSourceFile(file, overrides) && fs.existsSync(path.join(rootDir, file)));
    files.forEach((file, index) => onProgress?.(index + 1, file));
    return files;
  }
...
export async function scanDirectoryAsync(
  ...
  // Custom extension → language overrides from the project's codegraph.json.
  const overrides = loadExtensionOverrides(rootDir);
  if (fs.admittedSourceFiles(rootDir)) return scanDirectory(rootDir, onProgress);
...
export class ExtractionOrchestrator {
  ...
  async indexAll(
    ...
    const useWorker = packageExistsSync(parseWorkerPath);
    ...
    if (
      ...
      packageExistsSync(storeWorkerPath)
    ) {
  ...
  async sync(
    ...
    // A changed admission can retire paths absent from a watcher event list.
    if (fs.admittedSourceFiles(this.rootDir)) scopedPaths = undefined;
