...
export interface KernelModule {
  ...
  /** Isolated maintenance bootstrap only; older kernels must refuse that mode. */
  acquireMaintenanceWriter?(canonicalLockFile: string): boolean;
...
function candidatePaths(): string[] {
  // The entry bundle and kernel are staged together; no host override/fallback.
  return [path.join(__dirname, 'codegraph-kernel.node')];
}
