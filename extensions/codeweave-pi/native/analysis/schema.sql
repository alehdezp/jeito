...
-- Bindings may precede encoding; features survive transactional node replacement.
-- Publication owns recipe compatibility and pruning, not a cascading foreign key.
CREATE TABLE IF NOT EXISTS semantic_features (
    input_digest TEXT PRIMARY KEY NOT NULL,
    vector BLOB NOT NULL CHECK (typeof(vector) = 'blob' AND length(vector) = 1024)
);
