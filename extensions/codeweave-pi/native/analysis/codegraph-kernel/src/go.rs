...
impl<'t> Walker<'t> {
    ...
    // Go grouped type specs own their own comments; only an ungrouped spec
    // inherits the declaration's preceding comment. Keep this Go-local.
    fn declaration_docstring(&self, node: Node<'t>) -> Option<String> {
        let anchor = if node.kind() == "type_spec" {
            node.parent().filter(|parent| {
                parent.kind() == "type_declaration"
                    && !(0..parent.child_count()).any(|i| {
                        parent.child(i).is_some_and(|child| child.kind() == "(")
                    })
            }).unwrap_or(node)
        } else {
            node
        ...
        while let Some(comment) = sibling {
            ...
            // A preceding declaration's trailing comment is not documentation
            // for this declaration, even without a blank line between them.
            let previous = comment.prev_named_sibling();
            if previous.is_some_and(|prev| prev.end_position().row == comment.start_position().row) {
                break;
            }
            comments.push(crate::docstring::clean_comment_markers(self.text(comment)));
    ...
    fn extract_field(&mut self, node: Node<'t>) {
        // Anonymous nested structs have no represented owner. Still traverse
        // their types for existing references, but do not attach their fields
        // to the outer named struct just because it remains on the stack.
        let Some(struct_type) = node.parent().and_then(|list| list.parent()) else { return };
        if struct_type.kind() != "struct_type" { return; }
        let Some(spec) = struct_type.parent() else { return };
        if spec.kind() != "type_spec"
            || spec.child_by_field_name("type") != Some(struct_type)
            || !self.stack.last().is_some_and(|scope| {
                scope.kind == "struct" && scope.name == self.extract_name(spec)
            })
        {
            return;
        }
        // Embedded types have no field_identifier and remain extends evidence.
        // One node per declared name shares the full declaration, including tags.
        for i in 0..node.named_child_count() {
            let Some(name) = node.named_child(i) else { continue };
            if name.kind() != "field_identifier" { continue; }
            let name = self.text(name).to_string();
            self.create_node("property", &name, node, Extra {
                signature: Some(self.text(node).to_string()),
                docstring: self.declaration_docstring(node),
                is_exported: Some(name.as_bytes().first().is_some_and(u8::is_ascii_uppercase)),
                ..Extra::default()
            });
        }
    }
    ...
    fn extract_type_alias(&mut self, node: Node<'t>) -> bool {
        ...
        let docstring = self.declaration_docstring(node);
    ...
    fn extract_go_interface_methods(&mut self, interface_type: Node<'t>, iface_row: u32, iface_name: &str) {
        ...
        for i in 0..interface_type.named_child_count() {
            ...
            if !mname.is_empty() {
                ...
                self.create_node("method", &mname, m, Extra {
                    signature,
                    docstring: self.declaration_docstring(m),
                    ..Extra::default()
                });
            }
        }
