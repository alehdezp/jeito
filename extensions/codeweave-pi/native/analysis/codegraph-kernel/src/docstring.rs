...
#[path = "comment_ranges.rs"]
mod comment_ranges;
pub use comment_ranges::preceding_comment_ranges;
...
pub fn preceding_docstring(node: Node, src: &str) -> Option<String> {
    let ranges = preceding_comment_ranges(node);
    if ranges.is_empty() { return None; }
    Some(ranges.into_iter().map(|range| clean_comment_markers(&src[range]))
        .collect::<Vec<_>>().join("\n").trim().to_string())
}
