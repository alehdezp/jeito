//! Semantic projection helpers. Source classification, binding names, field
//! bodies and lexical owners are shared with live extraction in source.rs;
//! framework-specific roles and emission filters remain here.
...
impl<'t> Walker<'t> {
    ...
    pub(super) fn extract_function(&mut self, node: Node<'t>, name_override: Option<String>) {
        ...
        // The anonymous initializer's binding is shared source fact, not a
        // separately guessed declaration name.
        if name_override.is_none() && name == "<anonymous>" {
            if let Some(binding) = super::source::function_binding_name(node, self.src) {
    ...
    fn react_component_hoc(&self, value: Node<'t>) -> Option<Option<Node<'t>>> {
        ...
        if let Some(args) = value.child_by_field_name("arguments") {
            for i in 0..args.named_child_count() {
                if let Some(a) = args.named_child(i as _) {
    ...
    fn extract_react_component_node(
        ...
        // The retained component projection represents this exact render
        // callable, not every anonymous descendant of the initializer.
        self.projected_callables.push((super::source::identity(inner), row));
    ...
    pub(super) fn extract_class(&mut self, node: Node<'t>) {
        ...
        for i in 0..body.named_child_count() {
            if let Some(c) = body.named_child(i as _) {
                self.visit_node(c);
            }
        }
        self.stack.pop();
    ...
    pub(super) fn extract_method(&mut self, node: Node<'t>) {
        ...
        if let Some(callable) = super::source::field_callable(node) {
            self.projected_callables.push((super::source::identity(callable), row));
        }
    ...
    pub(super) fn extract_enum(&mut self, node: Node<'t>) {
        ...
        for i in 0..body.named_child_count() {
            let Some(child) = body.named_child(i as _) else { continue };
        ...
        let name_node = node
            ...
            .or_else(|| {
                (0..node.named_child_count())
                    .filter_map(|i| node.named_child(i as _))
    ...
    pub(super) fn extract_variable(&mut self, node: Node<'t>) {
        ...
        for i in 0..node.named_child_count() {
            let Some(child) = node.named_child(i as _) else { continue };
    ...
    fn extract_rtk_hook_bindings(&mut self, pattern: Node<'t>, is_exported: bool) {
        for i in 0..pattern.named_child_count() {
            let Some(binding) = pattern.named_child(i as _) else { continue };
    ...
    pub(super) fn extract_object_literal_functions(&mut self, obj: Node<'t>) {
        for i in 0..obj.named_child_count() {
            let Some(member) = obj.named_child(i as _) else { continue };
            if member.kind() == "pair" {
    ...
    fn find_initializer_returned_object(&self, call: Node<'t>, depth: u32) -> Option<Node<'t>> {
        ...
        for i in 0..args.named_child_count() {
            let Some(arg) = args.named_child(i as _) else { continue };
    ...
    fn function_returned_object(&self, fn_node: Node<'t>) -> Option<Node<'t>> {
            ...
            match n.kind() {
                ...
                "parenthesized_expression" => {
                    for i in 0..n.named_child_count() {
                        if let Some(inner) = n.named_child(i as _).and_then(as_object) {
                            return Some(inner);
                        }
                    }
                    None
        ...
        if body.kind() == "statement_block" {
            for i in 0..body.named_child_count() {
                let Some(stmt) = body.named_child(i as _) else { continue };
                if stmt.kind() != "return_statement" {
                    continue;
                }
                for j in 0..stmt.named_child_count() {
                    if let Some(obj) = stmt.named_child(j as _).and_then(as_object) {
                        return Some(obj);
                    }
                }
            }
    ...
    fn find_rtk_endpoints_object(&self, call: Node<'t>) -> Option<Node<'t>> {
        ...
        for i in 0..args.named_child_count() {
            ...
            for j in 0..arg.named_child_count() {
                let Some(member) = arg.named_child(j as _) else { continue };
                if member.kind() == "pair" {
    ...
    pub(super) fn extract_store_collection_methods(&mut self, config: Node<'t>) {
        for i in 0..config.named_child_count() {
            let Some(member) = config.named_child(i as _) else { continue };
            if member.kind() != "pair" {
                continue;
            }
            let Some(key) = member.child_by_field_name("key") else { continue };
    ...
    fn extract_pinia_setup_body(&mut self, setup: Node<'t>) {
        ...
        for i in 0..body.named_child_count() {
            ...
            if stmt.kind() == "function_declaration" {
                ...
                for j in 0..stmt.named_child_count() {
                    let Some(decl) = stmt.named_child(j as _) else { continue };
    ...
    fn extract_ts_type_alias_members(&mut self, value: Node<'t>, alias_row: u32, alias_name: &str) {
        ...
        if value.kind() == "object_type" {
            ...
            for i in 0..value.named_child_count() {
                if let Some(op) = value.named_child(i as _) {
        ...
        for obj_type in object_types {
            for i in 0..obj_type.named_child_count() {
                let Some(child) = obj_type.named_child(i as _) else { continue };
    ...
    fn extract_ts_tuple_contract_names(&mut self, value: Node<'t>, alias_row: u32, alias_name: &str) {
        ...
        for tuple in tuples {
            for i in 0..tuple.named_child_count() {
                let Some(entry) = tuple.named_child(i as _) else { continue };
                ...
                for j in 0..type_args.named_child_count() {
                    let Some(arg) = type_args.named_child(j as _) else { continue };
    ...
    fn is_ts_function_typed_property(&self, property_signature: Node) -> bool {
        ...
        for i in 0..type_anno.named_child_count() {
            if let Some(inner) = type_anno.named_child(i as _) {
    ...
    fn emit_import_binding_refs(&mut self, node: Node<'t>, from_row: u32) {
        ...
        for i in 0..clause.named_child_count() {
            let Some(child) = clause.named_child(i as _) else { continue };
            match child.kind() {
                "identifier" => push(self, Some(child)),
                "named_imports" => {
                    for j in 0..child.named_child_count() {
                        let Some(spec) = child.named_child(j as _) else { continue };
                ...
                "namespace_import" => {
                    let n = (0..child.named_child_count())
                        .filter_map(|k| child.named_child(k as _))
                        .find(|c| c.kind() == "identifier")
    ...
    pub(super) fn emit_re_export_refs(&mut self, node: Node<'t>) {
        ...
        let imports_kind = edge_kind_index("imports").unwrap();
        for i in 0..clause.named_child_count() {
            let Some(spec) = clause.named_child(i as _) else { continue };
        ...
        if let Some(func) = func {
            if func.kind() == "member_expression" {
                ...
                if let Some(property) = property {
                    ...
                    } else if receiver.is_some_and(|r| r.kind() == "member_expression") {
                        // Keep the field receiver: this.db.exec is not this.exec.
    ...
    /// Persist only parser-proven local default bindings. Export statements
    /// name an existing source declaration; they do not create another node.
    pub(super) fn emit_default_export(&mut self, root: Node<'t>) {
        let has_token = |node: Node, kind: &str| {
            (0..node.child_count()).any(|i| node.child(i as _).is_some_and(|c| c.kind() == kind))
        };
        let mut defaults = Vec::new();
        for i in 0..root.named_child_count() {
            let Some(statement) = root.named_child(i as _) else { continue };
            if statement.kind() != "export_statement"
                || statement.child_by_field_name("source").is_some()
                || has_token(statement, "type")
            {
                continue;
            }
            if has_token(statement, "default") {
                defaults.push((statement, statement.child_by_field_name("declaration")
                    .or_else(|| statement.child_by_field_name("value"))));
            }
            for j in 0..statement.named_child_count() {
                let Some(clause) = statement.named_child(j as _) else { continue };
                if clause.kind() != "export_clause" { continue; }
                for k in 0..clause.named_child_count() {
                    let Some(specifier) = clause.named_child(k as _) else { continue };
                    if specifier.kind() != "export_specifier" || has_token(specifier, "type") { continue; }
                    // JS represents `as default` as an anonymous keyword,
                    // while TS exposes it through the named alias field.
                    let is_default = specifier.child_by_field_name("alias")
                        .is_some_and(|alias| matches!(alias.kind(), "identifier" | "default")
                            && self.text(alias) == "default")
                        || (has_token(specifier, "as") && has_token(specifier, "default"));
                    if is_default {
                        defaults.push((statement, specifier.child_by_field_name("name")));
                    }
                }
            }
        }
        let [(statement, Some(binding))] = defaults.as_slice() else { return };
        let identity = if binding.kind() == "identifier" {
            // Source callable ownership is not JS block-binding scope. Require
            // an actual module declaration (or its export/variable wrapper),
            // then use the canonical identity; never bind a same-named block local.
            let facts = super::source::collect(root, self.src);
            let mut declarations = facts.regions.iter().filter(|region| {
                if region.parent.is_some() || region.name.as_deref() != Some(self.text(*binding)) {
                    return false;
                }
                (0..root.named_child_count()).any(|i| {
                    let Some(top) = root.named_child(i as _) else { return false };
                    let declaration = if top.kind() == "export_statement" {
                        top.child_by_field_name("declaration").unwrap_or(top)
                    } else { top };
                    if super::source::identity(declaration) == region.id { return true; }
                    is_variable_type(declaration.kind()) &&
                        (0..declaration.named_child_count()).any(|j| {
                            declaration.named_child(j as _)
                                .is_some_and(|node| super::source::identity(node) == region.id)
                        })
                })
            });
            let Some(declaration) = declarations.next() else { return };
            if declarations.next().is_some() { return; }
            declaration.id.clone()
        } else if binding.child_by_field_name("name").is_some()
            && matches!(super::source::classify(binding.kind()),
                Some(super::source::Kind::Class | super::source::Kind::Function))
        {
            super::source::identity(*binding)
        } else {
            // Calls, anonymous expressions and imported bindings need facts we
            // do not own here. In particular, no text regex supplies a target.
            return;
        };
        let mut rows = self.source_ids.iter().enumerate()
            .filter(|(_, source)| source.as_ref() == Some(&identity))
            .map(|(row, _)| row as u32);
        let Some(target) = rows.next() else { return };
        if rows.next().is_some() { return; }
        let metadata = self.arena.put(r#"{"exportedName":"default"}"#);
        self.tables.push_edge(&crate::buffers::EdgeRow {
            source_idx: 0,
            target_idx: target,
            kind: edge_kind_index("exports").unwrap(),
            provenance: 1, // tree-sitter
            line: self.line_of(*statement),
            column: self.col_of(*statement),
            metadata_json: metadata,
            source_id_str: crate::buffers::NONE_STR,
            target_id_str: crate::buffers::NONE_STR,
        });
    }
    ...
    pub(super) fn extract_decorators_for(&mut self, decl: Node<'t>, decorated_row: u32) {
        ...
        for i in 0..decl.named_child_count() {
            let Some(child) = decl.named_child(i as _) else { continue };
            self.consider_decorator(child, decorated_row);
            if child.kind() == "modifiers" {
                for j in 0..child.named_child_count() {
                    if let Some(m) = child.named_child(j as _) {
        ...
        for i in 0..parent.named_child_count() {
            if let Some(sib) = parent.named_child(i as _) {
        ...
        if decl_idx > 0 {
            ...
            while j >= 0 {
                let Some(sib) = parent.named_child(j as _) else {
                    j -= 1;
                    continue;
                };
    ...
    fn consider_decorator(&mut self, n: Node<'t>, decorated_row: u32) {
        ...
        for i in 0..n.named_child_count() {
            let Some(child) = n.named_child(i as _) else { continue };
        ...
        for i in 0..node.named_child_count() {
            ...
            match child.kind() {
                ...
                "implements_clause" | "class_interface_clause" | "super_interfaces" | "interfaces" => {
                    for j in 0..child.named_child_count() {
                        if let Some(iface) = child.named_child(j as _) {
