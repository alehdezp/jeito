#!/usr/bin/env python3
"""
Benchmark runner for pi-nav performance evaluation.
...
def _pi_nav_version() -> Optional[str]:
    """Get installed pi-nav version."""
    try:
        result = subprocess.run(
            [str(Path.home() / ".cargo" / "bin" / "pi-nav"), "--version"],
            capture_output=True, text=True, timeout=5,
        )
        return result.stdout.strip().removeprefix("pi-nav ") if result.returncode == 0 else None
...
def run_single(
    ...
    return {
        ...
        "pi_nav_version": _pi_nav_version() if "pi_nav" in mode_name else None,
