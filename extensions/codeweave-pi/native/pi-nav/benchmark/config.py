...
# MCP config arguments for codex (pi-nav server)
PI_NAV_MCP_CODEX_ARGS = [
    "-c", f'mcp_servers.pi_nav.command="{Path.home()}/.cargo/bin/pi-nav"',
    "-c", 'mcp_servers.pi_nav.args=["--mcp", "--edit"]',
]


@dataclass
class ModeConfig:
    """Configuration for a benchmark mode (baseline vs pi-nav)."""
    name: str
    tools: list[str]
...
PI_NAV_MCP_CONFIG = FIXTURES_DIR / "pi_nav_mcp.json"
REPOS_DIR = Path("/tmp/pi_nav_bench/repos")
...
MODES = {
    ...
    "pi_nav": ModeConfig(
        name="pi_nav",
        tools=["Read", "Edit", "Grep", "Glob", "Bash"],
        mcp_config_path=str(PI_NAV_MCP_CONFIG),
        description="Built-in tools + pi-nav MCP (hybrid)",
    ),
    "pi_nav_forced": ModeConfig(
        name="pi_nav_forced",
        tools=["Read", "Edit"],
        mcp_config_path=str(PI_NAV_MCP_CONFIG),
        description="pi-nav MCP only (no Bash/Grep/Glob)",
    ),
}
