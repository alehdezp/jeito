...
| Model | pi_nav_search/run | pi_nav_read/run | pi_nav_files/run | Host tools/run | Adoption rate |
|---|---|---|---|---|---|
...
- **pi_nav** — Built-in tools + pi-nav MCP server (hybrid mode)
- **pi_nav_forced** — pi-nav MCP + Read/Edit only (Bash, Grep, Glob removed)
...
- pi-nav installed (`cargo install --path .`)
...
python benchmark/run.py --tasks all --repos ripgrep,fastapi,gin,express --models haiku --reps 1 --modes pi_nav_forced
