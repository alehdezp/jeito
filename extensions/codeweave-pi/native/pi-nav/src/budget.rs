...
/// `pi_nav_read full:true`, `regex`, or `diff` can't blow the host budget.
pub const DEFAULT_BUDGET: u64 = 24_000;
pub const MAX_BUDGET: u64 = DEFAULT_BUDGET;

pub fn clamp(requested: u64) -> u64 {
    requested.min(MAX_BUDGET)
}
...
mod tests {
    ...
    #[test]
    fn requested_budget_never_exceeds_the_hard_response_cap() {
        assert_eq!(clamp(1_000), 1_000);
        assert_eq!(clamp(u64::MAX), MAX_BUDGET);
    }
}
