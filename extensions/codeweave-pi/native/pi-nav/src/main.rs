...
/// pi-nav — Tree-sitter indexed lookups, smart code reading for AI agents.
/// One tool replaces `read_file`, grep, glob, `ast_grep`, and find.
#[derive(Parser)]
#[command(name = "pi-nav", version, about)]
struct Cli {
    #[command(subcommand)]
    ...
    /// Enable edit mode: hashline output + pi_nav_write tool.
    #[arg(long)]
    edit: bool,
...
enum Command {
    /// Private supervised maintenance entrypoint; never used by live queries.
    #[command(hide = true)]
    UpdateIndex,
    /// Install pi-nav into an MCP host's config.
...
fn main() {
    pi_nav::configure_thread_pools();
    let cli = Cli::parse();

    // Shell completions
    if let Some(shell) = cli.completions {
        clap_complete::generate(shell, &mut Cli::command(), "pi-nav", &mut io::stdout());
        return;
    }
    ...
    if let Some(cmd) = cli.command {
        match cmd {
            Command::UpdateIndex => {
                if let Err(error) = update_index() {
                    eprintln!("[pi-nav:maintenance] {error}");
                    process::exit(1);
                }
            }
            Command::Install { ref host, edit } => {
                if let Err(e) = pi_nav::install::run(host, edit) {
            ...
            Command::Overview => {
                ...
                let output = pi_nav::overview::fingerprint(&cwd);
                if output.is_empty() {
            ...
            Command::Grok {
                ...
                match pi_nav::run_grok(&target, &scope, full) {
            ...
            Command::Diff {
                ...
                let diff_source = match pi_nav::diff::resolve_source(
                    Some(&source),
                    a_str.as_deref(),
                    b_str.as_deref(),
                    patch_str.as_deref(),
    ...
    if cli.mcp {
        if cli.no_overview {
            std::env::set_var("PI_NAV_NO_OVERVIEW", "1");
        }
        ...
        if let Err(e) = pi_nav::mcp::run(cli.edit, mcp_scope.as_deref()) {
    ...
    // Map mode
    if cli.map {
        let cache = pi_nav::cache::OutlineCache::new();
        let scope = cli.scope.canonicalize().unwrap_or(cli.scope);
        let output = pi_nav::map::generate(&scope, 3, cli.budget, &cache);
    ...
    let query = if let Some(q) = cli.query {
        ...
        eprintln!("usage: pi-nav <query> [--scope DIR] [--section N-M] [--budget N]");
        process::exit(3);
    };
    ...
    if cli.deps {
        ...
        let result = pi_nav::run_deps(&path, &scope, cli.budget);
        emit_result(result, &query, cli.json, is_tty);
        return;
    }
...
/// A newline-delimited request followed by an OPEN stdin pipe. EOF means the owner
/// died or revoked the job, not that preparation should continue detached.
fn update_index() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    if io::stdin().is_terminal() {
        return Err("update-index requires a supervised parent pipe".into());
    }
    let mut parent = io::BufReader::new(io::stdin());
    let mut request = String::new();
    parent.by_ref().take(65_537).read_line(&mut request)?;
    if request.len() > 65_536 || !request.ends_with('\n') {
        return Err("missing or oversized native update request".into());
    }
    let request: pi_nav::index::evidence::UpdateRequest = serde_json::from_str(&request)?;
    std::thread::Builder::new()
        .name("updater-parent".into())
        .spawn(move || {
            let _ = io::copy(&mut parent, &mut io::sink());
            // SQLite rolls an interrupted transaction back on recovery. Do not rely on
            // parent-side timers: those disappear in a parent crash.
            process::exit(75);
        })?;
    let status = pi_nav::index::evidence::reconcile(
        &request,
...
fn emit_result(
    result: Result<String, pi_nav::error::TilthError>,
    query: &str,
    json: bool,
    is_tty: bool,
) {
