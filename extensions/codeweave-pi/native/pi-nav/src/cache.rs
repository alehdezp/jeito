...
impl OutlineCache {
    ...
    /// Parse an already-read current source snapshot without rereading the path.
    /// The caller supplies the metadata identity captured with that source.
    pub(crate) fn get_or_parse_source(
        &self,
        path: &Path,
        mtime: SystemTime,
        content: Arc<String>,
        ...
        let key = path.to_path_buf();
        if let Some(entry) = self.parsed.get(&key) {
            if entry.mtime == mtime && Arc::ptr_eq(&entry.file.content, &content) {
        ...
        let mut parser = tree_sitter::Parser::new();
        parser.set_language(&ts_lang).ok()?;
        let tree = parser.parse(content.as_str(), None)?;
        let file = Arc::new(ParsedFile {
            content,
            tree,
            lang,
