...
use super::{ChangeType, DiffSymbol, MatchConfidence, SymbolChange, SymbolIdentity, SymbolSnapshot};
...
pub(crate) fn match_symbols(old: &[DiffSymbol], new: &[DiffSymbol]) -> Vec<SymbolChange> {
    ...
    for (id, old_indices) in &old_by_id {
        if let Some(new_indices) = new_by_id.get(id) {
            ...
            for (oi, ni) in pair_overloads(old, new, old_indices, new_indices) {
                ...
                let change = if o.content_hash == n.content_hash {
                ...
                changes.push(symbol_change(Some(o), Some(n), change, MatchConfidence::Exact));
            }
        ...
        if let Some(new_idxs) = struct_new.get(key) {
            if old_idxs.len() == 1 && new_idxs.len() == 1 {
                ...
                changes.push(symbol_change(
                    Some(&old[oi]), Some(&new[ni]),
                    ChangeType::Renamed { old_name: old[oi].identity.name.clone() },
                ...
                for &ni in new_idxs {
                    if !new_matched[ni] {
                        new_matched[ni] = true;
                        changes.push(symbol_change(None, Some(&new[ni]), ChangeType::Added,
                            MatchConfidence::Ambiguous(count)));
                    }
                }
                for &oi in old_idxs {
                    if !old_matched[oi] {
                        old_matched[oi] = true;
                        changes.push(symbol_change(Some(&old[oi]), None, ChangeType::Deleted,
                            MatchConfidence::Ambiguous(count)));
                    }
                }
            }
    ...
    for (oi, ni, score) in candidates {
        ...
        changes.push(symbol_change(Some(&old[oi]), Some(&new[ni]), change,
    ...
    for (i, matched) in old_matched.iter().enumerate() {
        if !matched {
            changes.push(symbol_change(Some(&old[i]), None, ChangeType::Deleted,
                MatchConfidence::Exact));
        }
    }
    for (i, matched) in new_matched.iter().enumerate() {
        if !matched {
            changes.push(symbol_change(None, Some(&new[i]), ChangeType::Added,
                MatchConfidence::Exact));
        }
    }

    changes.sort_by(|a, b| a.line.cmp(&b.line).then_with(|| a.name.cmp(&b.name)));
    changes
}
...
fn symbol_change(
    old: Option<&DiffSymbol>,
    new: Option<&DiffSymbol>,
    change: ChangeType,
    match_confidence: MatchConfidence,
) -> SymbolChange {
    let selected = new.or(old).expect("a change has at least one source side");
    let snapshot = |symbol: &DiffSymbol| SymbolSnapshot {
        identity: symbol.identity.clone(),
        start_line: symbol.entry.start_line,
        end_line: symbol.entry.end_line,
    };
    let size = |symbol: &DiffSymbol| symbol.entry.end_line.saturating_sub(symbol.entry.start_line) + 1;
    SymbolChange {
        name: selected.identity.name.clone(),
        kind: selected.identity.kind,
        change,
        match_confidence,
        line: selected.entry.start_line,
        old_sig: old.and_then(|symbol| symbol.entry.signature.clone()),
        new_sig: new.and_then(|symbol| symbol.entry.signature.clone()),
        size_delta: Some((old.map_or(0, size), new.map_or(0, size))),
        old: old.map(snapshot),
        new: new.map(snapshot),
        diff_lines: Vec::new(),
    }
}
