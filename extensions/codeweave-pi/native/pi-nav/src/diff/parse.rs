    ...
    for line in raw.lines() {
        ...
        if current_hunk.is_none() {
            if let Some(src) = line.strip_prefix("--- ") {
                if src == "/dev/null" { status = FileStatus::Added; }
                continue;
            }
            if let Some(dst) = line.strip_prefix("+++ ") {
                if dst == "/dev/null" { status = FileStatus::Deleted; }
                continue;
            }
        }
...
mod tests {
    ...
    #[test]
    fn header_shaped_source_rows_keep_their_coordinates() {
        let files = parse_unified_diff("diff --git a/x b/x\n--- a/x\n+++ b/x\n@@ -20,2 +30,2 @@\n--- old source\n+++ new source\n context\n");
        let lines = &files[0].hunks[0].lines;
        assert_eq!(lines.len(), 3);
        assert_eq!((lines[0].old_line, lines[0].content.as_str()), (Some(20), "-- old source"));
        assert_eq!((lines[1].new_line, lines[1].content.as_str()), (Some(30), "++ new source"));
        assert_eq!((lines[2].old_line, lines[2].new_line), (Some(21), Some(31)));
    }
...
";
        ...
        let coordinates: Vec<_> = h.lines.iter().map(|line| (line.old_line, line.new_line)).collect();
        assert_eq!(coordinates, vec![(Some(1), Some(1)), (Some(2), None), (None, Some(2)), (None, Some(3)), (Some(3), Some(4))]);
    }
