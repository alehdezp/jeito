        ...
        "export_statement" => {
            ...
            for child in node.children(&mut cursor) {
                if let Some(mut inner) = node_to_entry(child, lines, lang, depth) {
                    ...
                    // The comment is a sibling of the export wrapper, not of
                    // its inner declaration. Preserve it on the real-kind
                    // entry so outline consumers (including fuzzy BM25F) see
                    // the same documentation as unexported declarations.
                    if inner.doc.is_none() {
                        inner.doc = extract_doc(node, lines);
                    }
                    return Some(inner);
                }
            }
...
/// Byte-slice one source line, clamping to UTF-8 char boundaries so a tree-sitter
/// byte column landing inside a multi-byte character can never panic.
...
/// Extract a function signature (name + params + return type) as a single line.
///
/// The signature is the source text from the node's start up to — but excluding —
/// its body, sliced at the body's exact start (row AND column) and folded onto one
/// line. Cutting at the body position (not the first `{`) preserves any brace that
/// belongs to the signature itself — e.g. an object-typed parameter
/// `f(params: { … }) {` — which a first-`{` truncation would cut off mid-params.
/// Multi-line parameter lists therefore fold complete; single-line signatures are
/// unchanged. When a grammar has no `body` field (declarations, traits, Elixir
/// `do` blocks) this falls back to the first line, truncated at the opening brace.
fn extract_signature(node: tree_sitter::Node, lines: &[&str]) -> String {
    ...
    let raw: String = match body {
        Some(body) => {
            let b = body.start_position();
            let b_row = b.row.min(lines.len() - 1);
            if b_row <= start_row {
                // Body opens on the first signature line — take the prefix before it.
                slice_line(lines[start_row], start.column, b.column).to_string()
            } else {
                let mut parts: Vec<String> = Vec::new();
                parts.push(slice_line(lines[start_row], start.column, usize::MAX).to_string());
                for r in (start_row + 1)..b_row {
                    parts.push(lines[r].to_string());
                }
                parts.push(slice_line(lines[b_row], 0, b.column).to_string());
                parts.join(" ")
            }
        }
        None => lines[start_row].to_string(),
    };
    // Fold interior newlines/indent into single spaces.
    let line = raw.split_whitespace().collect::<Vec<_>>().join(" ");
    let line = line.trim();
    // With a body field the slice already ends before the body brace, so any `{`
    // that remains belongs to the signature (object-typed params) and is kept —
    // trim only a stray trailing one. Without a body field we can't locate the
    // brace precisely, so cut at the first `{` (original behavior).
    let line: &str = if body.is_some() {
    ...
    // Safe for other languages: C/Java/Go/Rust hit the `{` handling above,
