...
pub(crate) fn read_current_bytes(path: &Path, section: bool) -> Result<Vec<u8>, TilthError> {
    let bytes = fs::read(path).map_err(|source| TilthError::IoError {
        path: path.to_path_buf(),
        source,
    })?;
    #[cfg(test)]
    READ_COUNT_PROBE.with(|probe| {
        if let Some((full, sections)) = probe.get() {
            probe.set(Some(if section {
                (full, sections + 1)
            } else {
                (full + 1, sections)
            }));
...
#[cfg(test)]
thread_local! {
    static READ_COUNT_PROBE: Cell<Option<(usize, usize)>> = const { Cell::new(None) };
}

#[cfg(test)]
pub(crate) fn with_read_count_probe<T>(operation: impl FnOnce() -> T) -> (T, (usize, usize)) {
    READ_COUNT_PROBE.with(|probe| {
        assert!(
            probe.replace(Some((0, 0))).is_none(),
            "read probe must not be nested"
        );
    });
    let result = operation();
    let counts = READ_COUNT_PROBE.with(|probe| probe.take().expect("read probe remained active"));
    (result, counts)
}
...
/// Override with `PI_NAV_FULL_SIZE_CAP` env var (bytes). Default: 2MB.
fn full_read_size_cap() -> u64 {
    std::env::var("PI_NAV_FULL_SIZE_CAP")
        .ok()
...
pub fn read_file(
    ...
    read_file_prepared(path, section, full, cache, edit_mode).map(|prepared| prepared.rendered)
}

/// Read and render one file while retaining the exact operation-local text.
/// The file bytes are loaded once and reused for rendering, typed metadata, and proof snapshots.
pub fn read_file_prepared(
    path: &Path,
    section: Option<&str>,
    full: bool,
    cache: &OutlineCache,
    edit_mode: bool,
) -> Result<PreparedRead, TilthError> {
    ...
    if meta.is_dir() {
        return list_directory(path).map(|rendered| PreparedRead {
            rendered,
            source_text: None,
        });
    }
    ...
    if byte_len == 0 {
        return Ok(PreparedRead {
            rendered: format::file_header(path, 0, 0, ViewMode::Empty),
            source_text: Some(String::new()),
        });
    }

    let buf = read_current_bytes(path, section.is_some())?;
    if crate::lang::detection::is_binary(&buf) {
        return Ok(PreparedRead {
            rendered: format::binary_header(path, byte_len, mime_from_ext(path)),
            source_text: None,
        });
    }
    let exact_text = String::from_utf8(buf.clone()).ok();

    if let Some(range) = section {
        let rendered = read_ranges_from_bytes(path, &buf, &[range], edit_mode, None)?;
        return Ok(PreparedRead {
            rendered,
        ...
        || crate::lang::detection::is_generated_by_content(&buf)
    {
        ...
        return Ok(PreparedRead {
            rendered: format::file_header(path, byte_len, line_count, ViewMode::Generated),
            source_text: exact_text,
        });
    }
            ...
            && crate::lang::detection::is_minified_by_content(&buf))
    {
        let line_count = memchr::memchr_iter(b'\n', &buf).count() as u32 + 1;
        return Ok(PreparedRead {
            rendered: format::file_header(path, byte_len, line_count, ViewMode::Minified),
            source_text: exact_text,
        });
    }
    ...
    if full && byte_len > cap {
        ...
        return Ok(PreparedRead {
            rendered: format!("{header}\n\n> **full=true skipped**: file is {file_mb:.1}MB (cap: {cap_mb:.1}MB). Use `section` to read specific ranges, or set PI_NAV_FULL_SIZE_CAP={byte_len} to override.\n\n{outline}"),
            source_text: exact_text,
        });
    }
...
pub fn read_ranges(path: &Path, ranges: &[&str], edit_mode: bool) -> Result<String, TilthError> {
    let bytes = read_current_bytes(path, true)?;
    read_ranges_from_bytes(path, &bytes, ranges, edit_mode, None)
}

/// Like [`read_ranges`], but with a strict shared token budget across all sections.
#[cfg(test)]
pub fn read_ranges_with_budget(
    ...
    let bytes = read_current_bytes(path, true)?;
    read_ranges_from_bytes(path, &bytes, ranges, edit_mode, Some(budget))
}

pub(crate) fn read_ranges_from_bytes(
    path: &Path,
    bytes: &[u8],
    ranges: &[&str],
    edit_mode: bool,
    ...
    let (blocks, total_bytes, total_lines) = collect_blocks_from_buf(bytes, ranges, edit_mode)?;
    let header = format::file_header(path, total_bytes, total_lines, ViewMode::Section);
    if blocks.len() == 1 {
        let single = format!("{header}\n\n{}", blocks[0].text);
        return Ok(budget.map_or(single.clone(), |limit| crate::budget::apply(&single, limit)));
    }

    let mut out = String::with_capacity(header.len() + total_bytes as usize + blocks.len() * 32);
    out.push_str(&header);
    let mut remaining =
        budget.map(|limit| limit.saturating_sub(estimate_tokens(header.len() as u64)));
    for (index, block) in blocks.iter().enumerate() {
        let chunk = format!(
            "\n\n─── lines {}-{} ───\n{}",
            block.start, block.end, block.text
        );
        if let Some(tokens_left) = remaining {
            let tokens = estimate_tokens(chunk.len() as u64);
            if tokens > tokens_left {
                if index == 0 {
                    return Ok(crate::budget::apply(
                        &format!("{header}{chunk}"),
                ...
                if marker_tokens <= tokens_left {
                    out.push_str(&marker);
                    remaining = Some(tokens_left - marker_tokens);
                    continue;
                }
                break;
            }
            remaining = Some(tokens_left - tokens);
        }
        out.push_str(&chunk);
    }
    Ok(out)
...
mod tests {
    ...
    fn full_true_size_cap_returns_outline() {
        ...
        let path = std::env::temp_dir().join("pi_nav_test_large.rs");
        ...
        std::env::set_var("PI_NAV_FULL_SIZE_CAP", "100");
        ...
        std::env::remove_var("PI_NAV_FULL_SIZE_CAP");
    ...
    fn read_ranges_disjoint_two_blocks() {
        let path = write_temp(
            "pi_nav_test_ranges_disjoint.txt",
            "l1\nl2\nl3\nl4\nl5\nl6\nl7\nl8\n",
        );
    ...
    fn read_ranges_preserves_user_order() {
        ...
        let path = write_temp("pi_nav_test_ranges_order.txt", "a\nb\nc\nd\ne\nf\n");
    ...
    fn read_ranges_overlap_is_emitted_verbatim() {
        ...
        let path = write_temp("pi_nav_test_ranges_overlap.txt", "x1\nx2\nx3\nx4\nx5\n");
    ...
    fn read_ranges_invalid_second_range_fails_whole_call() {
        let path = write_temp("pi_nav_test_ranges_invalid.txt", "one\ntwo\nthree\n");
    ...
    fn read_ranges_empty_input_errors() {
        let path = write_temp("pi_nav_test_ranges_empty.txt", "a\nb\n");
    ...
    fn read_ranges_edit_mode_emits_hashlines_per_block() {
        ...
        let path = write_temp(
            "pi_nav_test_ranges_edit_mode.txt",
            "alpha\nbeta\ngamma\ndelta\nepsilon\nzeta\n",
        );
    ...
    fn budgeted_passes_through_when_budget_ample() {
        let path = write_temp(
            "pi_nav_test_budgeted_passthrough.txt",
            "alpha\nbeta\ngamma\ndelta\nepsilon\nzeta\n",
        );
    ...
    fn budgeted_omits_later_sections_when_exhausted() {
        ...
        let path = write_temp("pi_nav_test_budgeted_omit.txt", &body);
        let budget = 240u64;
        let out =
    ...
    fn budgeted_truncates_first_section_when_too_large() {
        ...
        for i in 0..200 {
            let _ = writeln!(body, "line-{i}-padding-padding-padding");
        }
        let path = write_temp("pi_nav_test_budgeted_first_too_large.txt", &body);
        let budget = 100u64;
    ...
    fn budgeted_preserves_user_order_until_exhaustion() {
        ...
        let body = format!("{big_line}\n").repeat(30);
        let path = write_temp("pi_nav_test_budgeted_order.txt", &body);
        let budget = 240u64;
        let out =
    ...
    fn budgeted_never_exceeds_budget_even_when_markers_do_not_fit() {
        ...
        let body = format!("{big_line}\n").repeat(30);
        let path = write_temp("pi_nav_test_budgeted_strict_cap.txt", &body);
        let budget = 80u64;
        let out =
            read_ranges_with_budget(&path, &["1-3", "10-12", "20-22"], false, budget).unwrap();
        assert!(
    ...
    fn ogate_flat_file_returns_full_when_outline_barely_compresses() {
        ...
        let path = write_temp("pi_nav_ogate_flat.rs", &src);
    ...
    fn ogate_large_compressible_file_returns_outline() {
        ...
        let path = write_temp("pi_nav_ogate_heavy.rs", &src);
