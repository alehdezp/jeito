...
use tools::{tool_definitions, tool_write};
...
impl Services {
    fn new(edit_mode: bool, root: &Path) -> io::Result<Self> {
        let native =
            NativeSession::new(root, true).map_err(|error| io::Error::other(error.to_string()))?;
        Ok(Self {
...
// Runtime guidance lives only in prompts/mcp-base.md and prompts/mcp-edit.md;
// contributor guidance in AGENTS.md is intentionally independent.
const SERVER_INSTRUCTIONS: &str = include_str!("../../prompts/mcp-base.md");
const EDIT_MODE_EXTRA: &str = include_str!("../../prompts/mcp-edit.md");

/// MCP server over stdio. When `edit_mode` is true, exposes `pi_nav_write` and
/// switches `pi_nav_read` to hashline output format. Read-only deployments
/// (no `--edit`) omit `pi_nav_write` and its large schema entirely, so they pay
/// no edit-protocol context tax.
///
/// `scope` overrides the default search root. When provided, pi-nav chdir's to it
/// at startup so all tools, git commands, and searches use the correct project root.
/// This fixes MCP hosts that launch pi-nav with cwd=/ (e.g., Codex).
pub fn run(edit_mode: bool, scope: Option<&Path>) -> io::Result<()> {
    ...
    let services = Services::new(edit_mode, &std::env::current_dir().unwrap_or_default())?;
    let stdin = io::stdin();
    ...
    for line in stdin.lock().lines() {
            ...
            if msg.get("id") == Some(roots_id) {
                ...
                if !scope_is_explicit {
                    if let Some(root_path) = extract_root_from_response(&msg) {
                        ...
                        let _ = services.native.set_root(&root_path);
                    }
                }
                continue;
            }
        ...
        if method == "initialize" && !scope_is_explicit && pending_roots_id.is_none() {
            ...
            if client_caps.get("roots").is_some() {
                let roots_id = Value::String("pi_nav_roots_1".to_string());
...
fn handle_request(req: &JsonRpcRequest, services: &Services) -> JsonRpcResponse {
    ...
    match req.method.as_str() {
        "initialize" => {
            let overview = if std::env::var("PI_NAV_NO_OVERVIEW").is_ok() {
                String::new()
            } else {
                let cwd = std::env::current_dir().unwrap_or_default();
...
fn dispatch_tool(
    ...
    if tool == "pi_nav_write" && services.edit_mode() {
        return tool_write(args, &services.native.session, &services.native.bloom)
            .map(|text| ToolOutput::text(tool, text))
            .map_err(ToolFailure::error);
    }
    let format = if services.edit_mode() {
        ReadFormat::Hashline
    } else {
        ReadFormat::Plain
    };
    let mut context = OperationContext::for_session(&services.native, format, false);
    context.deadline = deadline;
    dispatch_read_only(tool, args, &services.native, &context).map_err(|error| match error {
        crate::dispatch::NativeError::Deadline => ToolFailure::deadline(error.to_string()),
        crate::dispatch::NativeError::Cancelled => ToolFailure::cancelled(error.to_string()),
        _ => ToolFailure::error(error.to_string()),
    })
}
...
impl ToolFailure {
    ...
    fn deadline(text: String) -> Self {
        Self {
            text,
            reason: IncompleteReason::Deadline,
        }
    }
...
fn handle_tool_call(req: &JsonRpcRequest, services: &Services) -> JsonRpcResponse {
    ...
    let result = if services.tracker().is_at_cap() {
        Err(ToolFailure::error(
            "server busy: too many prior operations still running after timeout. Wait or set PI_NAV_TIMEOUT=<seconds> higher."
                .into(),
        ))
    } else {
...
fn run_tool_with_timeout(
    ...
    match outcome {
        Ok(inner) => inner,
        Err(SpawnFailure::Timeout) => Err(ToolFailure::deadline(format!(
            "tool timed out after {}s — the operation took too long. Try: reduce scope, use section instead of full, or set PI_NAV_TIMEOUT=<seconds> to increase the limit.",
            timeout.as_secs()
        ))),
        Err(SpawnFailure::Panic) => Err(ToolFailure::error("tool panicked during execution".into())),
    }
}

fn build_tool_response(
    id: Option<Value>,
    result: Result<ToolOutput, ToolFailure>,
) -> JsonRpcResponse {
...
mod tests {
    ...
    #[test]
    fn structured_sidecar_preserves_existing_tool_text_bytes() {
        let root = tempfile::tempdir().unwrap();
        std::fs::write(root.path().join("a.rs"), "fn a() {}\n").unwrap();
        let args = serde_json::json!({"pattern": "*.rs", "scope": root.path()});
        let expected = crate::ops::tool_files_output(&args, None).unwrap().text;
        let services = Services::new(false, root.path()).unwrap();
        let output = dispatch_tool("pi_nav_files", &args, &services, None).unwrap();
        assert_eq!(output.text.as_bytes(), expected.as_bytes());
        assert_eq!(output.structured["schemaVersion"], 1);
    }
    ...
    fn server_instructions_byte_lock() {
        ...
        assert!(SERVER_INSTRUCTIONS
            .starts_with("pi-nav — code intelligence MCP server. Replaces grep, cat, find, ls"));
        assert!(SERVER_INSTRUCTIONS
        ...
        assert!(SERVER_INSTRUCTIONS
            .contains("To check what changed, use pi_nav_diff instead of Bash(git diff/git log)."));
    ...
    fn edit_mode_extra_byte_lock() {
        ...
        assert!(
            EDIT_MODE_EXTRA.starts_with("\n\npi_nav_write replaces the host Edit and Write tools."),
        ...
        assert!(EDIT_MODE_EXTRA
            .ends_with("DO NOT use the host Edit or Write tool. Use pi_nav_write for all writes."));
        assert!(
