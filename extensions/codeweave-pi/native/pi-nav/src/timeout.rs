...
/// Override with `PI_NAV_TIMEOUT` env var (seconds). Default: 90s.
pub(crate) fn request_timeout() -> Duration {
    let secs = std::env::var("PI_NAV_TIMEOUT")
        .ok()
        ...
        default(timeout) => {
            ...
            if coord.claim_timeout() {
                ...
                if n == ABANDONED_THREAD_WARN {
                    eprintln!(
                        "pi-nav: warning: {n} abandoned threads still running. \
                         Consider reducing scope or increasing PI_NAV_TIMEOUT."
                    );
                }
            }
...
mod tests {
    ...
    /// race on process-global `PI_NAV_TIMEOUT`. Any test that mutates this
    ...
    fn request_timeout_reads_env() {
        ...
        std::env::set_var("PI_NAV_TIMEOUT", "7");
        assert_eq!(request_timeout(), Duration::from_secs(7));
        std::env::remove_var("PI_NAV_TIMEOUT");
