...
pub(crate) fn tool_savings(_args: &Value, session: &Session) -> Result<String, String> {
...
pub(crate) fn tool_savings_output(
    args: &Value,
    session: &Session,
) -> Result<crate::output::ToolOutput, String> {
    let text = tool_savings(args, session)?;
    let (baseline_tokens, saved_tokens) = session.savings();
    Ok(crate::output::ToolOutput::complete(
        "pi_nav_savings",
        text,
        json!({ "baselineTokens": baseline_tokens, "savedTokens": saved_tokens }),
        2,
        2,
    ))
}
