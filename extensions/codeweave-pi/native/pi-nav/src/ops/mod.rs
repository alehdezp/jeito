...
pub(crate) use deps::tool_deps_output;
pub(crate) use diff::tool_diff_output;
pub(crate) use files::tool_files_output;
pub(crate) use grok::tool_grok_output;
pub(crate) use ls::tool_ls_output;
pub(crate) use read::tool_read_output;
pub(crate) use savings::tool_savings_output;
pub(crate) use search::tool_search_output;
pub(crate) use session::tool_session_output;
pub(crate) use symbol::tool_symbol_range_output;
pub(crate) use read::markdown_projection;
pub(crate) use symbol::captured_symbol_projection;
...
pub(super) fn resolve_scope(
    ...
    // An omitted scope is the caller's supplied root when available; direct
    // standalone callers retain the historical relative current-directory value.
    if scope_arg.is_none() {
        return match root {
            Some(root) if root.is_absolute() => Ok((
                root.canonicalize().unwrap_or_else(|_| root.to_path_buf()),
                None,
            )),
            Some(root) => Err(format!("root must be absolute, got {}", root.display())),
            None => Ok((PathBuf::from("."), None)),
        };
    }
...
mod tests {
    ...
    fn resolve_scope_no_arg_no_root_defaults_to_cwd() {
        ...
        // session (e.g. a bare pi_nav_search/pi_nav_files call with no scope).
    ...
    #[test]
    fn resolve_scope_no_arg_uses_root() {
        let args = serde_json::json!({});
        let tmp = tempfile::tempdir().unwrap();
