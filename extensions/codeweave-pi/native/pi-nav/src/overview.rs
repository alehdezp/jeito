...
pub(crate) struct OverviewData {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) git: Option<GitFacts>,
}

pub(crate) struct OverviewOutput {
    pub(crate) text: String,
    pub(crate) data: OverviewData,
}
...
pub fn fingerprint(root: &Path) -> String {
    fingerprint_typed(root, None).text
}

pub(crate) fn fingerprint_typed(
    root: &Path,
    context: Option<&crate::dispatch::OperationContext>,
) -> OverviewOutput {
    let start = Instant::now();
    if context.is_some_and(|value| value.check().is_err()) {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        fingerprint_inner(root, context)
    }));
    let elapsed = start.elapsed();
    if elapsed.as_millis() > 250 {
        eprintln!(
            "[pi-nav] fingerprint took {}ms (>250ms budget)",
            elapsed.as_millis()
        );
    }
    result.unwrap_or(OverviewOutput {
        text: String::new(),
        data: OverviewData::default(),
    })
}
...
fn fingerprint_inner(
    ...
    if let Some(manifest) = find_manifest(root) {
        if let Some(info) = parse_manifest(root, &manifest) {
            ...
            if let Some(git) = &git {
                lines.push(format!("  git: {}", format_git(git)));
            }
    ...
    let mut entry_points: Vec<String> = walk
        .code_files
        .iter()
        .filter_map(|(path, _)| {
            let name = Path::new(path).file_stem()?.to_str()?;
            matches!(name, "main" | "index" | "app" | "server" | "lib")
                .then(|| path.replace('\\', "/"))
        })
        .collect();
    entry_points.sort();
    ...
    OverviewOutput {
        text: lines.join("\n"),
        data: OverviewData {
            primary_language: primary_lang.map(|language| lang_display_name(language).to_string()),
...
fn git_output(
    ...
    let local_deadline = Instant::now() + Duration::from_millis(200);
    let deadline = context
        .and_then(|value| value.deadline)
        .map_or(local_deadline, |value| value.min(local_deadline));
    let status = loop {
        if context.is_some_and(|value| value.check().is_err()) || Instant::now() >= deadline {
            let _ = child.kill();
            let _ = child.wait();
            let _ = reader.join();
            return None;
    ...
    let trimmed = String::from_utf8(bytes).ok()?.trim().to_string();
    (!trimmed.is_empty()).then_some(trimmed)
}

fn git_facts(root: &Path, context: Option<&crate::dispatch::OperationContext>) -> Option<GitFacts> {
    let branch = git_output(root, &["branch", "--show-current"], context)
        .or_else(|| git_output(root, &["rev-parse", "--short", "HEAD"], context));
    let dirty = git_output(root, &["status", "--porcelain"], context)
        .is_some_and(|value| !value.is_empty());
    branch.map(|branch| GitFacts {
        branch: Some(branch),
        dirty,
    })
}

fn format_git(git: &GitFacts) -> String {
...
mod tests {
    ...
    fn test_fingerprint_on_tilth() {
        ...
        assert!(output.contains("pi-nav"), "should find project name");
    ...
    fn test_manifest_parsing() {
        ...
        assert_eq!(info.name.as_deref(), Some("pi-nav"));
