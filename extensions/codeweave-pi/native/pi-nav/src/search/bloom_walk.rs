...
where
    ...
    S: AsRef<str>,
{
    read_with_sources(path, targets, bloom, max_size, None)
}

pub(super) fn read_with_sources<I, S>(
    path: &Path, targets: I, bloom: &BloomFilterCache, max_size: u64,
...
where I: IntoIterator<Item = S>, S: AsRef<str> {
    ...
    let content = match sources {
        Some(sources) => sources.read_text(path).ok()?.as_str().to_owned(),
