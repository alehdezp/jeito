...
pub(crate) fn fit_to_budget(
    ...
    fit_to_budget_with_selection(body, blocks, budget_tokens).0
}

/// Value-budget the rendered blocks and return the keep mask used for each
/// block. Typed producers use this mask to retain evidence only for blocks
/// that are actually present in the returned text.
pub(crate) fn fit_to_budget_with_selection(
    body: &str,
    blocks: &[(i64, usize, usize)],
    budget_tokens: u64,
) -> (String, Vec<bool>) {
    ...
    let keep_all = vec![true; blocks.len()];
    if estimate_tokens(body.len() as u64) <= budget_tokens {
        return (body.to_string(), keep_all);
    }
    ...
    let items: Vec<(i64, u64)> = blocks
        .iter()
        .map(|&(value, start, end)| (value, estimate_tokens(end.saturating_sub(start) as u64)))
        .collect();
    ...
    for (index, &(_, start, end)) in blocks.iter().enumerate() {
