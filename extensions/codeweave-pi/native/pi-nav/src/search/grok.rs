...
use crate::search::callees::{
    extract_callee_names, structural_declaration_candidates, DeclarationCandidate,
};
...
pub struct GrokResult {
    ...
    pub body: String,
    pub callees_internal: Vec<DeclarationCandidate>,
    ...
    pub delegate_body: Option<(DeclarationCandidate, String)>,
}
...
pub fn grok(
    ...
    let resolved = structural_declaration_candidates(&callee_names, &target.path, &content, bloom);
    ...
    let resolved: Vec<DeclarationCandidate> = resolved
        .into_iter()
    ...
    let lone_internal_callee: Option<DeclarationCandidate> = if total_callees_internal == 1 {
    ...
    // and tsconfig `baseUrl`/`paths` carriers of this same target. The binder
    // is a no-op for non-TS/JS targets and for ambiguous owner resolution.
    let alias_callers = crate::search::bindings::find_alias_callers_batch(
        &target.path,
        &[target.name.clone()],
        scope,
    )
    .into_iter()
    .map(|(local, ac)| (local, CallerMatch::from(ac)))
    .collect::<Vec<_>>();

    let prod_and_test: Vec<CallerMatch> =
        crate::search::callers::merge_caller_rows(raw_callers, alias_callers)
            .into_iter()
            .map(|(_, m)| m)
...
fn slice_body(content: &str, start_line: u32, end_line: u32, max_lines: usize) -> String {
    ...
    let _ = writeln!(
        out,
        "... ({elided} lines elided — use pi_nav_read for full body)"
    );
...
pub fn format_grok(result: &GrokResult, scope: &Path) -> String {
    ...
    if let Some((callee, callee_body)) = &result.delegate_body {
        ...
        let _ = writeln!(
            out,
            "\n## candidate body (name-only; delegation unverified) — {}:{}",
    ...
    if !result.callees_internal.is_empty() || !result.callees_external.is_empty() {
        ...
        let _ = writeln!(
            out,
            "\n## callees — source-name alternatives ({internal_count} declarations, {external_count} unmatched); physical-range origins, receivers and bindings unverified",
        );
        ...
        let mut by_file: std::collections::BTreeMap<&Path, Vec<&DeclarationCandidate>> =
        ...
        for ext in &result.callees_external {
            let _ = writeln!(out, "{ext:<20} no declaration candidate established");
        }
    ...
    if !result.callers.is_empty() {
        let _ = writeln!(
            out,
            "\n## callers — name/alias candidates; binding unverified ({})",
        ...
        for (file, mut callers) in by_file {
            ...
            for m in callers {
                if let Some(site) = &m.site {
                    let _ = writeln!(out, "    {}", site.caption(&m.content));
                } else {
...
";
    ...
    #[test]
    fn grok_recovers_alias_reexport_and_tsconfig_callers() {
        let tmp = tempfile::tempdir().unwrap();
        write_fixture(
            tmp.path(),
            "src/manifest.ts",
        ...
        .unwrap();
        // One exported function resolves to one definition, not two.
        assert_eq!(result.target.name, "loadManifest");
        assert_eq!(result.target.other_def_count, 0);
        assert!(result.target.path.ends_with("src/manifest.ts"));
        let by_owner: Vec<&CallerMatch> = result
            .callers
            .iter()
            .filter(|c| c.calling_function != "<top-level>")
            .collect();
        let owners: HashSet<&str> = by_owner
            .iter()
            .map(|c| c.calling_function.as_str())
            .collect();
        for owner in [
            ...
            assert!(
                ...
                "missing alias caller {owner}: {owners:?}"
            );
        }
        assert_eq!(result.total_callers, 4);
        let direct = by_owner
            .iter()
            .find(|c| c.calling_function == "directCaller")
            .unwrap();
        assert!(direct.path.ends_with("src/direct.ts"));
        assert_eq!(direct.line, 3);
        assert!(direct.call_text.contains("localLoad(raw)"));
        let rendered = format_grok(&result, tmp.path());
        assert!(
            rendered.contains("directCaller"),
            "caller must be visible in rendered text:\n{rendered}"
        );
    }
...
";
        ...
        // Each site retains the actual expression and immediate owner.
        assert!(
            callers_block.contains("caller_a [3]: call \"target()\""),
            "caller_a should appear: {callers_block}"
        );
        assert!(
            callers_block.contains("caller_b [4]: call \"target()\""),
...
";
        ...
        // Candidate-body context is useful without asserting proven delegation.
        let rendered = format_grok(&result, tmp.path());
        assert!(
            rendered.contains("## candidate body (name-only; delegation unverified)"),
            "formatted output must disclose the candidate-body status, got: {rendered}"
        );
        assert!(
            rendered.contains("step1 = x * 2"),
            "formatted output must contain callee implementation line, got: {rendered}"
        );
        // Candidate-body context precedes the names-only callee alternatives.
        let delegate_pos = rendered
            .find("## candidate body")
            .expect("## candidate body missing");
        let callees_pos = rendered.find("## callees").expect("## callees missing");
        assert!(
            delegate_pos < callees_pos,
            "candidate body must appear before callees section"
        );
    }
