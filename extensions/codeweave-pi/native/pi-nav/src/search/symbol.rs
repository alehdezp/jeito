...
pub(crate) fn search_with_sources(
    ...
    let definition_sources = sources.clone();
    let usage_sources = sources.clone();
...
/// Resolve `query` to its single current implementation owner (path +
/// canonical name) when exactly one code definition of that exact name
/// exists in scope. Returns `None` for ambiguity — two real same-named
/// implementations, overload signatures at distinct ranges, interface vs
/// implementation — and for absent definitions, so callers stay
/// lexical-only instead of guessing an owner. Markdown heading "definitions"
/// (weight 30) are not implementation owners and are excluded.
pub(crate) fn single_definition_owner(
    query: &str,
    scope: &Path,
) -> Result<Option<(PathBuf, String)>, TilthError> {
    ...
    let mut owners: Vec<_> = result
        .matches
        .iter()
        .filter(|m| m.is_definition && m.def_weight >= 40 && m.def_name.as_deref() == Some(query))
        .map(|m| {
            (
                m.path.clone(),
                m.def_name.clone().unwrap_or_else(|| query.to_string()),
                m.def_range,
                m.declaration.as_ref().map(|d| {
    ...
    owners.dedup();
    if owners.len() == 1 {
        let (path, name, _, _) = owners.remove(0);
        Ok(Some((path, name)))
    } else {
        Ok(None)
    }
...
fn find_defs_treesitter(
    ...
    if lang.is_some_and(super::declarations::supports) {
        for declaration in super::declarations::collect(root, content) {
            if declaration.region().name.as_deref() == Some(query) {
                defs.push(declaration.to_match(path, content, file_lines, mtime));
            }
            // Keep the existing syntactic implements projection, selecting its
            // class from canonical facts rather than a second declaration walk.
            if declaration.region().kind == crate::tsjs_source::Kind::Class {
                let bytes = &declaration.region().id.bytes;
                if let Some(node) = root.descendant_for_byte_range(bytes.start, bytes.end) {
                    if extract_implemented_interfaces(node, &lines)
                        .iter()
                        .any(|name| name == query)
                    {
                        let mut matched = declaration.to_match(path, content, file_lines, mtime);
                        matched.def_name = Some(format!(
                            "{} implements {query}",
                            declaration.region().name.as_deref().unwrap()
                        ));
                        matched.impl_target = Some(query.to_string());
                        matched.def_weight = 80;
                        defs.push(matched);
                    }
                }
            }
    ...
    dedup_export_wrappers(&mut defs);
    defs
}

/// Collapse `export_statement` wrapper rows into the same-name declaration
/// rows they contain. Identity-based, after collection: a wrapper row
/// (def_weight 30 — only `export_statement` maps to 30) is dropped only when
/// a higher-weight declaration row with the same `def_name` AND a range
/// inside the wrapper's range was actually collected from the same file.
/// That containment pair is the proof that the wrapper and the declaration
/// are one implementation. No kind/depth assumption: a wrapper whose nested
/// declaration was never collected (beyond the walk's depth reach, or a
/// parse shape that yields no declaration row) survives as its own row, and
/// re-export clauses (`export { foo }`) produce no extractable name and are
/// untouched. Sibling same-named declarations, overload signatures, and
/// interface-vs-function pairs have disjoint ranges and never collapse.
fn dedup_export_wrappers(defs: &mut Vec<Match>) {
    let keep: Vec<bool> = defs
        .iter()
        .enumerate()
        .map(|(wrapper_idx, wrapper)| {
            if wrapper.def_weight != 30 {
                return true;
            }
            let Some(wrapper_name) = wrapper.def_name.as_deref() else {
                return true;
            };
            !defs.iter().enumerate().any(|(other_idx, other)| {
                other_idx != wrapper_idx
                    && other.def_weight > wrapper.def_weight
                    && other.def_name.as_deref() == Some(wrapper_name)
                    && contained_in(other.def_range, wrapper.def_range)
            })
        })
        .collect();
    let mut idx = 0usize;
...
fn contained_in(inner: Option<(u32, u32)>, outer: Option<(u32, u32)>) -> bool {
    match (inner, outer) {
        (Some((inner_start, inner_end)), Some((outer_start, outer_end))) => {
...
fn walk_for_definitions(
    ...
    if DEFINITION_KINDS.contains(&kind) {
        // Check if this node defines the queried symbol. Every matching node
        // is collected — including `export_statement` wrappers — and the
        // wrapper/declaration double-count is collapsed later by range
        // containment identity in `dedup_export_wrappers`, never by an
        // unconditional kind/depth assumption here.
...
"#;
    ...
    #[test]
    fn exported_typescript_function_counts_one_definition() {
        // An `export function` is one implementation: the parser sees an
        // export_statement wrapper AND a nested function_declaration. Both
        // currently emit a definition Match, so a single exported function
        // double-counts. One implementation must resolve to one definition
        // identity, and the surviving identity must be the declaration
        // (weight 100), not the wrapper (weight 30).
        let code = "export function loadManifest(raw: string) {\n  return raw;\n}\n";
        let ts_lang =
            crate::lang::outline::outline_language(crate::types::Lang::TypeScript).unwrap();
        let lines = code.lines().count() as u32;
        let defs = find_defs_treesitter(
        ...
        let rows: Vec<_> = defs
            .iter()
            .map(|d| (d.line, d.def_range, d.def_weight))
            .collect();
        assert_eq!(
            defs.len(),
            1,
            "export wrapper + nested declaration must collapse to one definition: {rows:?}"
        );
        assert_eq!(defs[0].def_weight, 100);
    }

    #[test]
    fn exported_typescript_const_counts_one_definition() {
        let code = "export const UNTAGGED_REQUESTS_SQL = `SELECT foo`;\n";
        let ts_lang =
            crate::lang::outline::outline_language(crate::types::Lang::TypeScript).unwrap();
        let lines = code.lines().count() as u32;
        let defs = find_defs_treesitter(
        ...
        assert_eq!(defs.len(), 1, "export const must count one definition");
    }

    #[test]
    fn same_named_implementations_stay_distinct_and_ambiguous() {
        // Two real `function foo` declarations in one file are two owners:
        // the wrapper collapse only applies to a wrapper's own nested
        // declaration, never to sibling declarations.
        ...
        assert_eq!(
            defs.len(),
            2,
            "two same-named implementations stay distinct"
        );
        let ranges: Vec<_> = defs.iter().map(|d| d.def_range).collect();
        let distinct: std::collections::HashSet<_> = ranges.iter().collect();
        assert_eq!(
            ranges.len(),
            distinct.len(),
            "distinct ranges never collapse"
        );
    }

    #[test]
    fn interface_and_implementation_stay_distinct() {
        let code = "interface loadManifest { (raw: string): string }\nfunction loadManifest(raw: string) { return raw; }\n";
        let ts_lang =
            crate::lang::outline::outline_language(crate::types::Lang::TypeScript).unwrap();
        let lines = code.lines().count() as u32;
        let defs = find_defs_treesitter(
        ...
        assert_eq!(
            defs.len(),
            2,
            "interface declaration and function implementation are distinct owners"
        );
    }
    #[test]
    fn wrapper_without_collected_declaration_survives_dedup() {
        // Identity-based collapse proof at the dedup level: a wrapper row
        // (weight 30) survives unless a same-name higher-weight row with a
        // range inside the wrapper's range was actually collected. The
        // single-line `export function foo() {}` shape has identical wrapper
        // and declaration ranges, so containment is inclusive.
        ...
        let wrapper = Match {
            ...
            mtime: now,
            def_range: Some((1, 1)),
            def_name: Some("foo".to_string()),
            def_weight: 30,
            source_association: None,
            declaration: None,
            impl_target: None,
        };
        // Wrapper alone: no contained same-name declaration row exists (the
        // parse may not have reached it) — it must survive, not vanish.
        let mut alone = vec![wrapper.clone()];
        dedup_export_wrappers(&mut alone);
        assert_eq!(
            alone.len(),
            1,
            "wrapper without a collected declaration survives"
        );
        // Wrapper + its collected nested declaration: the declaration row
        // (weight 100) is the implementation owner; the wrapper collapses.
        let mut pair = vec![
            wrapper.clone(),
            Match {
                path: std::path::PathBuf::from("test.ts"),
        ...
        dedup_export_wrappers(&mut pair);
        assert_eq!(
            pair.len(),
            1,
            "wrapper collapses into its collected declaration"
        );
        assert_eq!(
            pair[0].def_weight, 100,
            "the declaration row is the survivor"
        );
        // Sibling same-named declaration (disjoint range) is a real second
        // owner and never collapses with the wrapper.
        let mut siblings = vec![
            wrapper,
            Match {
                path: std::path::PathBuf::from("test.ts"),
                line: 3,
                text: "function foo() { return 2; }".to_string(),
                is_definition: true,
                exact: true,
                file_lines: 3,
                mtime: now,
        ...
        dedup_export_wrappers(&mut siblings);
        assert_eq!(
            siblings.len(),
            2,
            "disjoint same-name declarations stay distinct"
        );
    }

    #[test]
    fn reexport_clause_creates_no_phantom_definition_or_loss() {
        // `export { foo }` has no extractable declaration name: it must not
        // produce a phantom definition row, and it must not swallow the real
        // same-name declaration elsewhere in the file.
        let ts_lang =
            crate::lang::outline::outline_language(crate::types::Lang::TypeScript).unwrap();
        ...
        assert_eq!(defs.len(), 1, "only the real declaration row survives");
        assert_eq!(defs[0].def_weight, 100);

        let bare = "export { foo };\n";
        let bare_defs = find_defs_treesitter(
            std::path::Path::new("test.ts"),
            "foo",
            &ts_lang,
            Some(crate::types::Lang::TypeScript),
            bare,
            bare.lines().count() as u32,
            SystemTime::now(),
        );
        assert!(
            bare_defs.is_empty(),
            "a bare re-export clause is not an implementation owner"
        );
    }

    #[test]
    fn overload_signature_ranges_never_collapse() {
        // Overload signatures and the implementation body are sibling
        // declarations at distinct ranges. Whatever the parser classifies as
        // definitions must keep one row per distinct range — the wrapper
        // collapse must never merge sibling ranges.
        ...
        assert_eq!(
            ranges.len(),
            distinct.len(),
            "overload declaration ranges must not collapse: {ranges:?}"
        );
    }

    #[test]
    fn single_definition_owner_resolves_one_exported_typescript_function() {
        ...
        .unwrap();
        let owner = single_definition_owner("loadManifest", dir.path()).unwrap();
        let (path, name) = owner.expect("one exported function owns the name");
        assert_eq!(name, "loadManifest");
        assert!(path.ends_with("src/manifest.ts"));
    }

    #[test]
    fn single_definition_owner_stays_ambiguous_for_two_implementations() {
        let dir = tempfile::tempdir().unwrap();
        let src = dir.path().join("src");
        std::fs::create_dir_all(&src).unwrap();
        std::fs::write(
        ...
        .unwrap();
        assert!(
            single_definition_owner("loadManifest", dir.path())
                .unwrap()
                .is_none(),
            "two real same-named implementations must stay ambiguous"
        );
    }
