...
In long sessions, working instructions can recede behind the current exchange. Rewriting the system prompt or an earlier message to bring them back would also change input a provider might reuse. **guidepin appends a reminder instead:** the suite keeps previous messages where they were and puts selected guidance after the new request.

This choice does not add memory. Pi already carries the instructions and conversation history. The reminder aims to make evidence, scope and current decisions prominent again without asking the person to repeat them. Whether that improves adherence has not been established, and recurring messages cost context. guidepin ships with jeito because its wording and cadence serve the [versioned working instructions](../../config/APPEND_SYSTEM.md); it is not a standalone Pi package.
